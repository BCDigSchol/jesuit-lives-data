...
//Runs when search control finds a result
	searchControlProvinces.on('search:locationfound', function(e) {
		...
		var choice = document.getElementById("searchtext18").value; //record user input from search box
		...
		var jesuitLives = L.geoJson(movementMapProvinces, { //filter geojson based on user input and record start and stop data and province data
			filter:
				function (feature, layer) {
					if (feature.properties.provinceFull == choice) {
						return (feature.properties.provinceFull == choice);
						}
				},
...
*/
  ...
  L.easyButton('<strong>Export Data</strong>', function(){
    ...
    var exportGroup = jesuitLivesGroup;
