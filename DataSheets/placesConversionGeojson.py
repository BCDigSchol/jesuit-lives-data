...
###########################################################################
##Process for converting json to geojson, checking for coordinates, and exporting, for "province" places information
df2 = pd.DataFrame(provinces)
print('We have {} rows'.format(len(df2)))
str(df2.columns.tolist())

#convert all dates and numbers from strings to numbers so they can be filtered
df2['provLong'] =  pd.to_numeric(df['provLong'],errors='coerce')
df2['provLat'] =  pd.to_numeric(df['provLat'],errors='coerce')

#choose which column headings to include in geojson
useful_cols2 = ['JesuitPlaceFull', 'provLat', 'provLong', 'Placeholder City', 'Placeholder Country', 'vowedHere', 'MissionDate(s)', 'ViceProvinceDate(s)', 'ProvinceDate(s)']
df_subset2 = df2[useful_cols2]

#drop places that do not have spatial data
df_geo2 = df_subset2.dropna(subset=['provLat', 'provLong'], axis=0, inplace=False)
print('We have {} geotagged rows with spatial data'.format(len(df_geo2)))

#make sure to set lat and long equal to the proper fields
def df_to_geojson2(df, properties, lat='provLat', lon='provLong'):
...
#set columns to be exported as properties
useful_columns2 = ['JesuitPlaceFull', 'provLat', 'provLong', 'Placeholder City', 'Placeholder Country', 'vowedHere', 'MissionDate(s)', 'ViceProvinceDate(s)', 'ProvinceDate(s)']
...
with open(output_filename, 'w', encoding='utf8') as output_file:
    output_file.write('var allProvinces = {};'.format(geojson_str2))
